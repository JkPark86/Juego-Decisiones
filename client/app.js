...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Elementos de la interfaz (NUEVOS - Para el botón)
    ...
    // Control del estado de la consola (REINTEGRADO)
    ...
    function showScreen(screenToShow) {
        ...
        // --- LÓGICA DEL BOTÓN Y CONSOLA (REINTEGRADA) ---
        if (screenToShow === gameScreen) {
            toggleConsoleButton.style.display = 'block'; // Muestra el botón solo durante el juego
            gameConsole.style.display = 'block'; // Asegura que la consola esté visible al iniciar la partida
            isConsoleVisible = true;
            toggleConsoleButton.textContent = '[ Ocultar Interfaz ]';
        } else {
            toggleConsoleButton.style.display = 'none'; // Oculta el botón en menús, créditos, etc.
        }
    }


    // --- LÓGICA DEL BOTÓN OCULTAR INTERFAZ (REINTEGRADA) ---
    ...
    // --- RENDERIZACIÓN DE DECISIONES Y FINALES ---
    ...
    function renderDecision1() { // E1-1
        ...
        `;
        decisionButtons.innerHTML = `
            <button data-choice="D1-A">A. Se levanta, quejándose del dolor de espalda post-resurreción. (Héroe por Obligación)</button>
            <button data-choice="D1-B">B. Se levanta, camina dos pasos y toca un Cristal Brillante en la pared. (Exploración Rápida)</button>
            <button data-choice="D1-C">C. Le grita a la Universo: ""¡Renuncio! ¿Dónde está la puerta de salida?"" (Ataque al Sistema)</button>
        `;
    ...
    function renderDecision2() { // N1-2
        ...
        `;
        decisionButtons.innerHTML = `
            <button data-choice="D2-A">A. Aceptas la espada, ignorando el peso. ""Vamos por ese tal Rey Demonio."" (Valiente)</button>
            <button data-choice="D2-B">B. Dejas la espada y dices: ""No puedo con esto. Intentaré usar mi 20% de carisma."" (Cobarde y Sincero)</button>
            <button data-choice="D2-C">C. Intentas vender la espada por internet. ""Podríamos comprar dos espadas más pequeñas."" (La Táctica Absurda)</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision3() { // N2-1
        ...
        `;
        decisionButtons.innerHTML = `
            <button data-choice="D3-A">A. El camino lento, al menos es seguro... o eso espero. (Prudente/Largo)</button>
            <button data-choice="D3-C">B. El atajo obvio, para terminar esto ya. (Cobarde/Trampa)</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision4() { // N2-2 (PA-2 y D4)
        ...
        `;
        decisionButtons.innerHTML = `
            <button id="d4-a-button" data-choice="D4-A">A. Lo dejas. El universo te dará lo que mereces, no lo que necesitas. (Valiente/Rechazo)</button>
            <button id="d4-b-button" data-choice="D4-B">B. Lo tomas. ¡Lo mío es mío! (Egoísta/Aceptación)</button>
        `;
    ...
    function renderDecision5() { // N2-3
        ...
        `;
        decisionButtons.innerHTML = `
            <button data-choice="D5-A">A. "Soy ${currentHeroName}. Puedo (probablemente) hacerlo." (Compromiso)</button>
            <button data-choice="D5-B">B. "No, no puedo. ¿Podemos renegociar los términos de mi resurrección?" (Retiro)</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision6() { // N3-4
        ...
        `;
        decisionButtons.innerHTML = `
            <button id="d6-a-button" data-choice="D6-A">A. Usa todo lo que le queda para destruir el objeto de manera épica y sacrificada. (El Mártir Dramático)</button>
            <button id="d6-b-button" data-choice="D6-B">B. Deja que la constelación se encargue de la 'basura demoníaca' y se queda a salvo. (El Sobreviviente)</button>
        `;
    ...
    function loadFinalScene(finalId) {
        ...
        // Lógica de narración de Finales (Se mantiene igual)
        if (finalId === 'F1') {
    ...
    // --- LÓGICA DE ESCENAS ESPECIALES (Las funciones se mantienen, solo se ajusta el target de texto) ---

    async function processPA1() {
        sceneTitleGame.textContent = "1.3 PA-1: El Flashback Inoportuno";
        narrativeTextGame.innerHTML = `<p>"Un fuerte golpe de estrés te activa un flashback inoportuno."</p><p>...Calculando tu destino...</p>`;
        decisionButtons.innerHTML = '';
    ...
    function renderDuelo(sceneId) {
        ...
        if (sceneId === 'N3-2') {
            ...
            choiceC = "C. (Réplica Vergonzosa) Tu voz se quiebra mientras bajas la mirada: '¿La mitad...? Con razón siento que dejé mi autoestima en la tumba...'"; 
            ...
            choiceC = "C. (Réplica Vergonzosa) Te miras las manos temblorosas y murmuras: '…okay, quizá sí necesito otro exorcismo emocional…'";
