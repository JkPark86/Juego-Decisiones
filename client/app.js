...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleDecision(event) {
        ...
        try {
            ...
            // Código para añadir la clase 'decision-button' a los botones dinámicos
            const buttons = decisionButtons.querySelectorAll('button');
            buttons.forEach(button => {
                // Asegura que los botones creados en JS tengan estilo
                button.classList.add('button', 'is-dark', 'decision-button'); 
            });
    ...
    // --- RENDERIZACIÓN DE DECISIONES Y FINALES (Añadido de clases CSS) ---

    function renderDecision1() { // E1-1
        sceneTitleGame.textContent = "Decisión 1: El Sentido del Deber";
        narrativeTextGame.innerHTML = `
            <p>"Moriste de forma impresionante,${currentHeroName}. Tanto que tu ego duplicó su tamaño, sorprendente para un falso héroe. Ahora, el 'Gran Mal' o como a él le gusta ser llamado, 'El asombroso e imparable REY DEMONIO' ha regresado, y nuestro presupuesto solo alcanzó para traerte de vuelta a ti."</p>
            <p class="constellation-voice">"Levántate. El universo espera. Y sinceramente, el papeleo es interminable."</p>
        `;
        decisionButtons.innerHTML = `
            <button class="button is-dark decision-button" data-choice="D1-A">A. Se levanta, quejándose del dolor de espalda post-resurreción.</button>
            <button class="button is-dark decision-button" data-choice="D1-B">B. Se levanta, camina dos pasos y toca un Cristal Brillante en la pared.</button>
            <button class="button is-dark decision-button" data-choice="D1-C">C. Le grita a la Universo: ""¡Renuncio! ¿Dónde está la puerta de salida?""</button>
        `;
    ...
    function renderDecision2() { // N1-2
        ...
        `;
        decisionButtons.innerHTML = `
            <button class="button is-dark decision-button" data-choice="D2-A">A. Aceptas la espada, ignorando el peso. ""Vamos por ese tal Rey Demonio.""</button>
            <button class="button is-dark decision-button" data-choice="D2-B">B. Dejas la espada y dices: ""No puedo con esto. Intentaré usar mi 20% de carisma.""</button>
            <button class="button is-dark decision-button" data-choice="D2-C">C. Intentas vender la espada por internet. ""Podríamos comprar dos espadas más pequeñas.""</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision3() { // N2-1
        ...
        `;
        decisionButtons.innerHTML = `
            <button class="button is-dark decision-button" data-choice="D3-A">A. El camino lento, al menos es seguro... o eso espero.</button>
            <button class="button is-dark decision-button" data-choice="D3-C">B. El atajo obvio, para terminar esto ya.</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision4() { // N2-2 (PA-2 y D4)
        ...
        `;
        decisionButtons.innerHTML = `
            <button id="d4-a-button" class="button is-dark decision-button" data-choice="D4-A">A. Lo dejas. El universo te dará lo que mereces, no lo que necesitas.</button>
            <button id="d4-b-button" class="button is-dark decision-button" data-choice="D4-B">B. Lo tomas. ¡Lo mío es mío!</button>
        `;
    ...
    function renderDecision5() { // N2-3
        ...
        `;
        decisionButtons.innerHTML = `
            <button class="button is-dark decision-button" data-choice="D5-A">A. "Soy ${currentHeroName}. Puedo (probablemente) hacerlo."</button>
            <button class="button is-dark decision-button" data-choice="D5-B">B. "No, no puedo. ¿Podemos renegociar los términos de mi resurrección?"</button>
        `;
        decisionButtons.querySelectorAll('button').forEach(button => {
            button.onclick = handleDecision;
        });
    ...
    function renderDecision6() { // N3-4
        ...
        `;
        decisionButtons.innerHTML = `
            <button id="d6-a-button" class="button is-dark decision-button" data-choice="D6-A">A. Usa todo lo que le queda para destruir el objeto de manera épica y sacrificada.</button>
            <button id="d6-b-button" class="button is-dark decision-button" data-choice="D6-B">B. Deja que la constelación se encargue de la 'basura demoníaca' y se queda a salvo.</button>
        `;
    ...
    function loadFinalScene(finalId) {
        ...
        // Botón de menú del final, usando clases de Bulma
        decisionButtons.innerHTML = '<button class="button is-info is-medium is-fullwidth" onclick="location.reload()">Volver al Menú Principal</button>';
    ...
    function processPA1_Stage1() {
        ...
        `;
        // Primer botón: Activa el proceso  y la siguiente escena

        decisionButtons.innerHTML = `<button id="activate-flashback" class="button is-primary is-fullwidth">Activar Flashback (Ver Recuerdo)</button>`;
        document.getElementById('activate-flashback').addEventListener('click', processPA1_Stage2);
    }
    
    /**
     * Llama al servidor para obtener el resultado, se renderiza la escena final del flashback.
     */
    async function processPA1_Stage2() {
        ...
        try {
            ...
            if (response.ok) {
                // 3. Renderizar la escena del flashback con el botón de avance 
    ...
    function renderFlashbackScene(sceneId, newPoints) {
        ...
        decisionButtons.innerHTML = `<button id="continue-button" class="button is-info is-fullwidth">Continuar al Acto 2: El Gran Mapa</button>`;
    ...
    async function processD4(choiceD4) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                decisionButtons.innerHTML = `<button id="continue-n23-button" class="button is-info is-fullwidth">Continuar a N2-3: El Juicio</button>`;
    ...
    function renderDuelo(sceneId) {
        ...
        // Botones del duelo
        decisionButtons.innerHTML = `
            <button id="duelo-a-button" class="button is-dark decision-button" data-choice="Duelo-A" data-key="A">${choiceA}</button>
            <button id="duelo-b-button" class="button is-dark decision-button" data-choice="Duelo-B" data-key="B">${choiceB}</button>
            <button id="duelo-c-button" class="button is-dark decision-button" data-choice="Duelo-C" data-key="C">${choiceC}</button>
        `;
    ...
    function startTypingQTE(choiceId, textToType, impactVH) {
        ...
        narrativeTextGame.innerHTML = `
            <p>Tienes <span id="qte-timer-display" class="has-text-info has-text-weight-bold">${QTE_DURATION_LOCAL} segundos</span> para teclear esta frase exactamente:</p>
            <blockquote id="qte-text" class="box">${textToType}</blockquote>
            <input type="text" id="qte-input" class="input is-medium" placeholder="Escribe aquí rápidamente..." autofocus>
            <p id="qte-timer" class="has-text-centered has-text-primary has-text-weight-bold"></p>
        `;
        decisionButtons.innerHTML = '';
        
        let timer = QTE_DURATION_LOCAL;
        const inputElement = document.getElementById('qte-input');
        const timerDisplayElement = document.getElementById('qte-timer-display');
        const timerElement = document.getElementById('qte-timer');

        timerElement.textContent = `Tiempo restante: ${QTE_DURATION_LOCAL}`;
        
        const interval = setInterval(() => {
            timer -= 1;
            timerElement.textContent = `Tiempo restante: ${timer}`;
            timerDisplayElement.textContent = `${timer}`;
    ...
    async function processDueloResult(choiceId, baseImpactVH, qteSuccess, choiceKey) {
        ...
        // El contenido de resultMessage usa las clases .success-message y .fail-message del CSS
        ...
        try {
            ...
            if (response.ok) {
                updateGameStatus(data.currentVH, data.extraLives);
                // Se añade el mensaje de resultado ANTES del botón de continuar
                narrativeTextGame.innerHTML = narrativeTextGame.innerHTML + '<br>' + resultMessage;
                ...
                if (data.isFinal) {
                    ...
                    decisionButtons.innerHTML = `<button id="continue-duelo-button" class="button is-info is-fullwidth">Continuar al Asalto ${data.currentAssault + 1}</button>`;
