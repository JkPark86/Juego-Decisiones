...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CONSTANTE DEL QTE MODIFICADA A 30 SEGUNDOS ---
    ...
    function stopAllAudio() {
        // Pausamos todas las pistas. No reiniciamos el tiempo (currentTime) para evitar fallos.
        audioMenu.pause();
        audioTj.pause();
        audioBf.pause();
    }

    // Función para reproducir sin reiniciar si ya está sonando
    function playTrack(trackElement) {
        // Si la pista ya está sonando, no hacemos nada para evitar interrupciones.
        ...
        // Reiniciamos solo si no está en el inicio (para evitar escuchar el inicio de golpe)
        if (trackElement.currentTime !== 0) {
             trackElement.currentTime = 0;
        }
        
        trackElement.play().catch(error => {
            console.log("Error al reproducir el audio: El navegador requiere interacción del usuario primero.", error);
        });
    }

    // Función de navegación simple para cambiar de pantalla (MODIFICADA para audio)
    function showScreen(screenToShow) {
        ...
        if (screenToShow === mainMenu || screenToShow === creditsScreen) {
            // Menú y Créditos usan 'menu.mp3'
            playTrack(audioMenu);
        } else if (screenToShow === startScreen) {
             // Pantalla de ingreso de nombre usa 'tj.mp3' (Aventura)
             playTrack(audioTj);
        }
    ...
    // INICIALIZACIÓN: Muestra el menú principal al inicio
    showScreen(mainMenu); 
    ...
    startGameButton.onclick = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // La música tj.mp3 ya está sonando desde showScreen(startScreen)
                
                loadScene(data.initialScene); 
            } else {
    ...
    async function loadScene(sceneId) {
        ...
        // AUDIO: Si NO es el Acto 3, la música de aventura (tj.mp3) se mantiene.
        // Esto también maneja la transición de vuelta de BF.mp3 si el jugador gana un asalto.
        ...
        if (sceneId === 'E1-1') { 
            ...
            await startAct3(); // Inicia la música de batalla aquí (BF.mp3)
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.isFinal) {
                    ...
                    // El audio no se interrumpe porque playTrack(audioTj) verifica si ya está sonando en loadScene
    ...
    function loadFinalScene(finalId) {
        // AUDIO: Detener toda la música al llegar al Final
        stopAllAudio(); 
        ...
        if (finalId === 'F1') {
            ...
            finalNarrative = "Has muerto, mejor suerte en tu próxima vida. (El Rey Demonio te humilló hasta el punto de la desintegración o tu VH era demasiado bajo/perdiste dos QTE. Un final patético.)";
        } else if (finalId === 'F9') {
            finalTitle = 'Final 9: Derrota Digna (Obsoleto)';
            finalNarrative = "Este final ya no debería ocurrir. Si lo ves, revisa la lógica del servidor.";
        } else if (finalId === 'F10') {
    ...
    // --- LÓGICA DE ESCENAS ESPECIALES (MANTENIDA) ---
    ...
    async function startAct3() {
        ...
        // AUDIO: Iniciar la música de Batalla (BF.mp3)
        playTrack(audioBf); 
    ...
    function renderDuelo(sceneId) {
        ...
        const assaultNumber = parseInt(sceneId.charAt(3)) - 1; 
    ...
    // --- MECÁNICA QTE (TIEMPO MODIFICADO A 30 SEGUNDOS) ---

    function startTypingQTE(choiceId, textToType, impactVH) {
        // Usa la constante QTE_DURATION = 30
        const QTE_DURATION_LOCAL = QTE_DURATION; 
        
        narrativeText.innerHTML = `
            <h3>¡QUICK TIME EVENT! (QTE)</h3>
            <p>Tienes **${QTE_DURATION_LOCAL} segundos** para teclear esta frase exactamente (sin comillas y con mayúsculas):</p>
    ...
    async function processDueloResult(choiceId, baseImpactVH, qteSuccess = true, choiceKey) {
        ...
        // Lógica de castigo/bonificación (solo se usa en el cliente para determinar el impacto a enviar)
        if (choiceKey === 'A' && !qteSuccess) {
            // Falla el Tipado (QTE) 
            finalImpactVH = -3; 
            extraLivesLost = 1;
            narrativeText.innerHTML += `<p class="fail-message">¡ERROR CRÍTICO de TIPADO! Tu lengua tropezó. Pierdes **1 Vida Extra** y tu temple se desmorona (-3 VH).</p>`;
        } else if (choiceKey === 'A' && qteSuccess) {
            narrativeText.innerHTML += `<p class="success-message">¡ÉXITO en el TIPADO! Tu réplica fue perfecta. Ganas (+${finalImpactVH} VH).</p>`;
        } else if (choiceKey === 'B') { 
            finalImpactVH = -1; 
            extraLivesLost = 1;
            narrativeText.innerHTML += `<p class="fail-message">Réplica inefectiva. El Rey Demonio se ríe. Pierdes **1 Vida Extra**.</p>`;
        } else if (choiceKey === 'C') { 
            finalImpactVH = -2; 
            extraLivesLost = 2;
            narrativeText.innerHTML += `<p class="fail-message">Réplica vergonzosa. El pánico te hace perder **2 Vidas Extra**.</p>`;
        }
