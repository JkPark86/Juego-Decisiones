...
const decisionMap = {
    ...
    // Acto 2 (N2-1)
    'D3-A': { impact: 0, nextScene: 'N2-2', final: null },   
    'D3-B': { impact: -1, nextScene: 'F5', final: 'F5' },     
...
app.post('/api/duelo-process-assault', async (req, res) => {
    ...
    try {
        ...
        // Lógica Final por Derrota (F8/F9)
        // Solo aplica si las vidas llegan a cero Y fue un intento fallido (F1 o F2)
        if (newLives === 0 && (choiceId.includes('F1') || choiceId.includes('F2'))) { 
            isFinal = true;
            if (newVH < 2) { // VH < 2 (Cobarde/Débil)
                nextScene = 'F8'; 
            } else { // VH >= 2 (Héroe/Fuerte)
                nextScene = 'F9'; 
            }
        } else if (currentScene === 'N3-4') {
            // Éxito total: Avanza a N3-4 para D6
            nextScene = 'N3-4'; 
        }
        ...
        res.status(200).json({ 
            ...
            currentVH: newVH,
            extraLives: newLives,
            currentAssault: parseInt(currentScene.slice(2, 3)) // De N3-2 a 2
        });
...
app.listen(PORT, () => {
    ...
    // Intentar conectar a la DB al inicio (la conexión fallará si la URI es incorrecta)
